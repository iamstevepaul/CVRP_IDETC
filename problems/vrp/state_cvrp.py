...
class StateCVRP(NamedTuple):
    ...
    def initialize(input, visited_dtype=torch.uint8):
        ...
        return StateCVRP(
            ...
            demand=demand,
            current_demand=torch.normal(demand, input['std']).to(device=loc.device),
            std=input['std'],
    ...
    def update(self, selected):
        ...
        prev_a = selected
        n_loc = self.current_demand.size(-1)  # Excludes depot
        ...
        selected_demand = self.current_demand[self.ids, torch.clamp(prev_a - 1, 0, n_loc - 1)]
        ...
        current_demand=torch.normal(self.demand, self.std).to(device=self.coords.device)


        return self._replace(
            prev_a=prev_a, used_capacity=used_capacity, visited_=visited_,
            lengths=lengths, cur_coord=cur_coord, current_demand=current_demand, i=self.i + 1
        )
    ...
    def get_mask(self):
        ...
        exceeds_cap = (self.current_demand[self.ids, :] + self.used_capacity[:, :, None] > self.VEHICLE_CAPACITY)
