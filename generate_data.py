...
def generate_vrp_data(dataset_size, vrp_size):
    ...
    return list(zip(
        np.random.normal(mu, sigma, size=(dataset_size, 2)).tolist(),  # Depot location
        np.random.normal(mu, sigma, size=(dataset_size, vrp_size, 2)).tolist(),  # Node locations
        np.random.normal(5, .70, size=(dataset_size, vrp_size)).astype(int).tolist(),  # Demand, uniform integer 1 ... 9
    ...
    for problem, distributions in problems.items():
        for distribution in distributions or [None]:
            for graph_size in opts.graph_sizes:
                ...
                print(dataset[0])
                filename = "data/vrp_gaussian_100.pkl"
